...
int main()
{
	...
	double Med = 0.0,e1 = 0.0,e2 = 0.0,zbrmin = 0.0, As2 = 0.0,n2 = 0.0,fi2 = 0.0;
	Med = Ned * etot;
	e1 = etot + 0.5*h - a1;
	e2 = etot - 0.5*h + a2;
	zbrmin = max((0.1*Ned)/ (fyd*1000), 0.002*b*h);
	...
	a2 = 0.041;
	/*KONCOWE OBLICZENIA*/ 
	double xeff = 0.0, As1 = 0.0, n1 = 0.0,fi1=0.0, rop = 0.0, Isp = 0.0, EIp = 0.0, NBp = 0.0, etap = 0.0, etotp = 0.0,Medp=0.0,propmim=0.0;
	xeff = d - sqrt(d*d - (2 * (Ned*e1 - fyd * 1000 * As2*(d - a2))) / (fcd * 1000 * b));
	if ((xeff > 2 * a2) && (xeff <= xefflim))
	{
		As1 = (-Ned + fcd * 1000 * b*xeff + fyd * 1000 * As2) / (fyd * 1000);
	}
	fi1 = 0.02;
